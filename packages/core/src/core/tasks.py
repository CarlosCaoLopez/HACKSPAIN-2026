...
- `extinguish`: una por FRENTE, no por celda. Un frente es una componente conexa
  (8 vecinas) de celdas `burning`; con una tarea por celda salían más de ochenta en
  cinco minutos, los dos camiones se plantaban en las dos primeras y ningún cambio
  de viento movía nada. El id (`task_front_<cx>_<cz>`, de la celda con que nació)
  sobrevive mientras el frente crece o se desplaza; cuando dos frentes se juntan
  queda la tarea más grave y la otra se cierra; cuando un frente se apaga entero,
  se cierra. Gravedad por amenaza a POIs con gente; `target_cell` es la celda del
  frente que un camión puede sofocar desde la carretera (`suppress_reach_m`) más
  cercana al POI amenazado, o si no hay ninguna a tiro, la que antes llegará a él
  (el camión la espera en la carretera como cortafuegos).
...
"""A menos de esto de un POI con gente, un frente es `high` aunque no venga hacia él."""

DOWNWIND_DISTANCE_M = 200.0
DOWNWIND_HIGH_DISTANCE_M = 400.0
DOWNWIND_HALF_ANGLE_DEG = 45.0
"""A sotavento (el viento empuja el frente hacia el POI) los radios se amplían."""
...
"""Ocho vecinas: el mismo vecindario con el que el fuego se propaga en el sim, así
que dos celdas que se contagian son siempre el mismo frente."""
...
def fronts(cells: dict[str, Cell]) -> list[list[Cell]]:
    """Componentes conexas (8 vecinas) de las celdas `burning`, cada una ordenada
    por id y la lista ordenada por su primera celda. Puro."""
...
def _extinguish(state: WorldState, graph: RoadGraph) -> list[Task]:
    ...
    for idx, comp in enumerate(comps):
        ...
        severity, target = _assess_front(comp, peopled, state.wind, graph, current)
        if keep is None:
            out.append(
                Task(
                    id=_fresh_front_id(target, comp, state),
                    kind="extinguish",
                    target_cell=target,
        ...
        elif keep.severity != severity or keep.target_cell != target:
            out.append(
                keep.model_copy(update={"severity": severity, "target_cell": target})
            )
...
def _component_of(cell_id: str | None, members: list[set[str]]) -> int | None:
    ...
    try:
        ...
        return None
    around = {f"cell_{cx + dx}_{cz + dz}" for dx, dz in NEIGHBOURS_8}
    best: int | None = None
    best_n = 0
    for idx, ids in enumerate(members):
        n = len(around & ids)
...
def _assess_front(
    ...
    graph: RoadGraph,
    current: str | None,
) -> tuple[TaskSeverity, str]:
    """(gravedad, celda objetivo) de un frente.

    Gravedad por la amenaza a POIs con gente: `critical` a ≤ 60 m o a sotavento a
    ≤ 200 m; `high` a ≤ 120 m o a sotavento a ≤ 400 m; `medium` si no; `low` si
    además ningún camión puede sofocarlo desde la carretera.
    ...
    severity = _front_severity(centers.values(), peopled, wind, bool(attackable))
    threatened = _threatened_poi(centers.values(), peopled, wind, graph)
...
def _front_severity(
    centers, peopled: list[POI], wind: Wind, attackable: bool
) -> TaskSeverity:
    high = False
    for poi in peopled:
        for x, z in centers:
            d = math.hypot(poi.x - x, poi.z - z)
            downwind = _downwind(x, z, poi, wind)
            if d <= CRITICAL_DISTANCE_M or (downwind and d <= DOWNWIND_DISTANCE_M):
                return "critical"
            if d <= HIGH_DISTANCE_M or (downwind and d <= DOWNWIND_HIGH_DISTANCE_M):
                high = True
    if high:
        return "high"
    return "medium" if attackable else "low"


def _threatened_poi(centers, peopled: list[POI], wind: Wind, graph: RoadGraph):
    """El POI con gente al que el frente llega antes con este viento (`fire_eta_s`
    desde su celda más favorable). Con viento 270 el fuego de la cresta va a Pueblo
    A; a 300, a Pueblo B: es lo que hace que el cambio de viento cambie el objetivo."""
    best: POI | None = None
