...
- **`fixtures/**` solo se añade.** Esto genera `run_fake_v6.jsonl` y no toca el golden ni
...
OUT = Path("fixtures/run_fake_v6.jsonl")
...
class Variation:
    ...
    **Los valores por defecto son los de hoy, byte a byte**: `Variation()` produce
    exactamente `run_fake_v6.jsonl`, y `test_es_determinista` lo comprueba contra el
