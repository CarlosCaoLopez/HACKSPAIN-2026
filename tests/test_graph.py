...
ROADS = [
    RoadEdge(id="wp_base-wp_cruce", a="wp_base", b="wp_cruce", length_m=100),
    RoadEdge(id="wp_cruce-wp_nor_01", a="wp_cruce", b="wp_nor_01", length_m=95),
    RoadEdge(id="wp_nor_01-wp_nor_02", a="wp_nor_01", b="wp_nor_02", length_m=100),
    RoadEdge(id="wp_nor_02-wp_pueblo", a="wp_nor_02", b="wp_pueblo", length_m=95),
    RoadEdge(id="wp_cruce-wp_sur_01", a="wp_cruce", b="wp_sur_01", length_m=100),
    RoadEdge(id="wp_sur_01-wp_pueblo", a="wp_sur_01", b="wp_pueblo", length_m=130),
]
...
def test_restore_devuelve_la_ruta_original(graph):
    graph.cut("wp_sur_01-wp_pueblo", "árbol caído")
    graph.restore("wp_sur_01-wp_pueblo")
    assert graph.shortest_path("wp_base", "wp_pueblo")[2] == "wp_sur_01"


def test_sin_ruta_viva_devuelve_none(graph):
    graph.cut("wp_sur_01-wp_pueblo", "fuego")
    graph.cut("wp_nor_02-wp_pueblo", "fuego")
...
def test_es_determinista_con_rutas_empatadas():
    ...
    roads = [
        RoadEdge(id="wp_a-wp_b", a="wp_a", b="wp_b", length_m=50),
        RoadEdge(id="wp_a-wp_c", a="wp_a", b="wp_c", length_m=50),
        RoadEdge(id="wp_b-wp_d", a="wp_b", b="wp_d", length_m=50),
        RoadEdge(id="wp_c-wp_d", a="wp_c", b="wp_d", length_m=50),
    ]
    rutas = {
...
def test_ids_desconocidos_fallan_claro(graph):
    ...
    with pytest.raises(ValueError, match="no existe la arista"):
        graph.cut("wp_fantasma-wp_nada", "x")
...
def test_una_arista_huerfana_falla_al_construir():
    ...
    with pytest.raises(ValueError, match="no es un waypoint"):
        RoadGraph(
            ...
            [RoadEdge(id="wp_a-wp_no_existe", a="wp_a", b="wp_no_existe", length_m=10)],
        )
...
def test_resolve_edge_acepta_las_formas_que_circulan(graph):
    ...
    las direcciona como `wp_a-wp_b`, y el backbone escribe `wp_a-wp_b` a
    secas. Las tres formas tienen que llegar a la misma arista."""
    esperado = "wp_sur_01-wp_pueblo"
...
def test_resolve_edge_devuelve_none_si_no_existe(graph):
    assert graph.resolve_edge("wp_a-wp_b") is None
...
def test_cortar_por_extremos_sin_conocer_el_id(graph):
    ...
    assert not graph.is_cut("wp_sur_01-wp_pueblo")
