...
def anchor(**over) -> GeoAnchor:
    base = {"id": "t", "place": "Sitio", "lat0": 40.0, "lon0": -4.0, "meters_per_block": 25}
...
def test_los_null_del_final_de_un_forecast_no_son_datos_rotos():
    data = {"hourly": {"time": ["2025-08-14T00:00", "2025-08-14T01:00"],
                       "wind_speed_10m": [3.0, None], "wind_direction_10m": [90, None]}}
...
def test_una_hora_ilegible_se_cuenta_y_no_tumba_las_demas():
    data = {"hourly": {"time": ["no-es-una-hora", "2025-08-14T01:00"],
                       "wind_speed_10m": [3.0, 3.0], "wind_direction_10m": [90, 90]}}
    parsed = parse_hourly(data)
...
def test_current_da_una_muestra():
    data = {"current": {"time": "2026-09-19T08:45", "wind_speed_10m": 1.32, "wind_direction_10m": 81}}
    parsed = parse_current(data)
    assert parsed.records == [WindSample(datetime(2026, 9, 19, 8, 45, tzinfo=UTC), 1.32, 81.0)]
...
def test_el_umbral_critico_es_60_y_esta_incluido():
    ...
    assert by_key(to_facts([sample(159)], anchor(), ctx))["wind:bearing_deg"].severity == "low"
    ctx = FeedContext()
    to_facts([sample(100)], anchor(), ctx)
    assert by_key(to_facts([sample(160)], anchor(), ctx))["wind:bearing_deg"].severity == "critical"
...
def test_la_misma_muestra_dos_ciclos_seguidos_no_se_republica():
    ctx = FeedContext()  # en vivo: cada ciclo trae la misma hora hasta que cambia el modelo
...
def test_el_giro_se_mide_contra_lo_ultimo_publicado_y_no_contra_la_muestra_anterior():
    ...
    to_facts([sample(3)], anchor(), ctx)  # no se publica: el último publicado sigue siendo 0
